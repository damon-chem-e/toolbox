function J = HJBmap2D_distance3(DD,J,r,u,mu,sig,par)
...
eta   = cell(1,par.N);
Seta  = NaN(1,par.N);
distance = par.maxP.*ones(1,par.N);
xi    = cell(1,par.N);
testD = cell(1,par.N);
...
distance = reshape(distance,[par.n1,par.n2]);
for i=1:par.n1
    for j=1:par.n2
        if and(i<par.n1/2,i<distance(i,j))
            distance(i,j)=i;
        end
        if and(j<par.n2/2,j<distance(i,j))
            distance(i,j)=j;
        end
        if and(i>par.n1/2,(par.n1-i)<distance(i,j))
            distance(i,j)=par.n1-i+1;
        end
        if and(j>par.n2/2,(par.n2-j)<distance(i,j))
            distance(i,j)=par.n2-j+1;
        end
    end 
end
distance = reshape(distance,[1,par.N]);
...
if par.ns>1
    for ik=1:par.N
        [eta{ik},xi{ik},testD{ik}] = decompfun_distance(Sig{ik},par,distance(ik));
...
for ik=1:par.N
    ...
    for id=1:par.ndim
        ...
        if id==1
            ...
            if rem((ik-1),mdim(2))==0
                flag2=1;
            end
        end
        if and(ik-mdim(id)>=1,flag2==0)
            A(ik,ik-mdim(id)) = A(ik,ik-mdim(id))+Dk(id,ik);
        end
        if and(ik+mdim(id)<=par.N,flag1==0)
    ...
    for ix=1:length(eta{ik})
        ...
        if and(ik-xdim>=1,and(ik-xdim<=par.N,flag2==0))
            A(ik,ik-xdim) = A(ik,ik-xdim)-eta{ik}(ix);
        end
        if and(ik+xdim>=1,and(ik+xdim<=par.N,flag1==0))
            A(ik,ik+xdim) = A(ik,ik+xdim)-eta{ik}(ix);
        end
    end
