function [eta,xi,testD,B] = decompfun_distance(A,par,distance)

maxP_specific = min(par.maxP,distance);
n = size(A,1);
...
%         v_xp = v_xp/n_xp;
        ...
        if or(p+pp>maxP_specific,norm(v_ah-v_ap)<=par.tolD*norm(v_ah))
